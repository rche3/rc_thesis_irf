**** JORDAGK_TWOINSTRUMENTS.DO

*** February 24, 2018
*** This program estimates IRFs and both 2-step and 1-step multipliers using the Jorda method
***	It constructs multipliers using both instruments: Military news variable and the Blanchard-Perotti shock
...
local shock newsy; /* since bp is added below, choose newsy here to consider both instruments */
...
outsheet h Fdifflin Fdiffrec Fdiffexp using junkfdiff.csv if h<=20, comma replace ;
outsheet h Fefflin Feffrec Feffexp using junkmp.csv if h<=20, comma replace ;

display as text "Montiel-Pflueger robust weak instrument test and TSLS critical values for tau=10%";

list h Fefflin Critlin Feffrec Critrec Feffexp Critexp Feffboth Critboth if h<=20;
outsheet h Critlin Critrec Critexp using junkcrit.csv if h<=20 , comma replace ;

list h multlin1 multrec1 multrecb multexp1 multexpb if h<=20;
outsheet h multlin1 multrec1 multrecb multexp1 multexpb using junk1step.csv if h<=20, comma replace ;


list h multlin1 seylin multrec1 seyrec multexp1 seyexp ptestdiff arp if h<=20;
outsheet h multlin1 multrec1 multexp1 using junkmultse.csv if h<=20, comma replace ;
