...
%               tabular array Tbl is Tbl.Properties.VariableNames. The
%               default for linear models Mdl is Mdl.CoefficientNames.
%
...
if isa(Data,'dataset')
    ...
    try
        ...
        error(message('econ:hac:DataNotConvertible'))
...
dispFlag = lower(parseObj.Results.display);
...
elseif ~isnumeric(Data) && ~isa(Data,'table') && ~isa(Data,'LinearModel') 

    error(message('econ:hac:DataFormatUnsupported'))
    
elseif isa(Data,'table') && (size(Data,2) < 2)

    error(message('econ:hac:DataWithoutResponse'))
...
if ~isnumeric(y) || ~isvector(y)
    ...
    error(message('econ:hac:ResponseNonNumericVector'))
...
if ~islogical(iFlag) || ~isscalar(iFlag)
    
    error(message('econ:hac:InterceptNonLogicalScalar'))
...
function OK = displayCheck(dispFlag)
    
if isnumeric(dispFlag) || ~isvector(dispFlag)

    error(message('econ:hac:DisplayFlagNonString'))

elseif ~ismember(lower(dispFlag),{'cov','full','off'})
