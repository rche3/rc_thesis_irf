...
%% plot linear VAR results, correctness of bootstrap
close all
fig = figure;

% Define colors
green_color = [0, 0.5, 0]; % Dark green
purple_color = [0.5, 0, 0.5]; % Dark purple

% set plot properties variables
subplot_title_size = 12;

% Top row subplots (green)
ax1 = subplot(2,3,1);
hold on
...
set_plot_properties(ax1, green_color, 'MATLAB IRF SEs - Monte Carlo', subplot_title_size)
...
legend('irf', 'lower', 'upper')
ylabel('IRF to Gov Spend')
set_plot_properties(ax2, green_color, 'STATA (RZ Original) IRF SEs - Analytical', subplot_title_size)
axis tight
...
legend('irf', 'lower', 'upper')
ylabel('IRF to Gov Spend')
set_plot_properties(ax3, green_color, 'MATLAB Custom Code - Lutkepohl (2000) Bootstrap', subplot_title_size)
axis tight

% Bottom row subplots (purple)
ax4 = subplot(2,3,4);
hold on
...
legend('irf', 'lower', 'upper')
ylabel('IRF to GDP')
set_plot_properties(ax4, purple_color, '', subplot_title_size)
...
legend('irf', 'lower', 'upper')
ylabel('IRF to GDP')
set_plot_properties(ax5, purple_color, '', subplot_title_size)
axis tight
...
legend('irf', 'lower', 'upper')
ylabel('IRF to GDP')
set_plot_properties(ax6, purple_color, '', subplot_title_size)
axis tight
...
% 0.2-0.3 range
%%
function set_plot_properties(ax, color, title_text, title_size)
    plots = get(ax, 'Children');
    for i = 1:length(plots)
        set(plots(i), 'LineWidth', 1.5, 'Color', color);
    end
    title(ax, title_text, 'FontSize', title_size);
end
