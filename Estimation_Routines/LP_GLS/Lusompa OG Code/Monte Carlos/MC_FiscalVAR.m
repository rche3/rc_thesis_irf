function [CoverageFiscalVAR, LengthFiscalVAR, BiasFiscalVAR] = MC_FiscalVAR(maxh,M,nstraps,long_auto)


%Monte Carlo Code for the Fiscal VAR in the paper.
...
%Output:
    %CoverageFiscalVAR is a (q*q)x(maxh) matrix of coverage estimates over all M Monte Carlos
    %LengthFiscalVAR is a  (q*q)x(maxh) matrix of average length over all M Monte Carlos
    %BiasFiscalVAR is a (q*q)x(maxh) matrix of average bias over all M Monte Carlos

    load('Fiscal Policy Data MC')

True_data=[RGDPPCGrowth RSpendPCGrowth]';
[q, T]=size(True_data); 
% names=char('Output Growth', 'Spending Growth'); 
...
BiasVAR_analytic(:,:,m)=BiasVAR_analytictemp;
     ...
     ['Monte Carlo Iteration ',int2str(m),' of ',int2str(M),' of Fiscal VAR']


end

 CoverageFiscalVAR=[mean(CoverageLP_GLS_boot,3) mean(CoverageLP_GLS_boot2,3) mean(CoverageLP_GLS_analytic,3) mean(CoverageLPHAR,3) mean(CoverageVAR_boot,3) mean(CoverageVAR_analytic,3)];
 
 LengthFiscalVAR=[mean(LengthLP_GLS_boot,3) mean(LengthLP_GLS_boot2,3) mean(LengthLP_GLS_analytic,3) mean(LengthLPHAR,3) mean(LengthVAR_boot,3) mean(LengthVAR_analytic,3)];
 
BiasFiscalVAR=[mean(BiasLP_GLS_boot,3) mean(BiasLP_GLS_boot2,3) mean(BiasLP_GLS_analytic,3) mean(BiasLPHAR,3) mean(BiasVAR_boot,3) mean(BiasVAR_analytic,3)];
