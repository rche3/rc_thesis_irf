function [Coveragearma, Lengtharma, Biasarma] = MC_arma(maxh,coef,M,nstraps,samplesize)

%Monte Carlo Code for the Kilian and Kim (2011) arma(1,1).
...
%Input:
    ...
    %coef is the chosen coefficient for the arma(1,1) (0, .25, .5, or .75 in the paper)
...
%Output:
    ...
    %Biasarma is a (q*q)x(maxh) matrix of average bias over all M Monte Carlos
...
modSim = arima('Constant',0,'AR',B1,'MA',M1,'Variance',Sigma);
...
BiasVAR_analytic(:,m)=BiasVAR_analytictemp;
     ...
     ['Monte Carlo Iteration ',int2str(m),' of ',int2str(M),' For arma M1 = ',num2str(coef)]

end


 Coveragearma=[mean(CoverageLP_GLS_boot,2) mean(CoverageLP_GLS_boot2,2) mean(CoverageLP_GLS_analytic,2) mean(CoverageLPHAR,2) mean(CoverageVAR_boot,2) mean(CoverageVAR_analytic,2)];
 
 Lengtharma=[mean(LengthLP_GLS_boot,2) mean(LengthLP_GLS_boot2,2) mean(LengthLP_GLS_analytic,2) mean(LengthLPHAR,2) mean(LengthVAR_boot,2) mean(LengthVAR_analytic,2)];
 
  Biasarma=[mean(BiasLP_GLS_boot,2) mean(BiasLP_GLS_boot2,2) mean(BiasLP_GLS_analytic,2) mean(BiasLPHAR,2) mean(BiasVAR_boot,2) mean(BiasVAR_analytic,2)];
