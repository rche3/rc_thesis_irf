...
% Hence we expect 3 seconds / DGP MonteCarlo Rep
% Fairly robust test is 1000 MC reps * 50 DGPs = 50,000 DGP MonteCarlo Reps
% 50,000 MCDGP * 3 sec / MCDGP = 150,000 seconds = 41.7 hours = 1.8 days
...
settings.specifications.random_n_spec         = num_DGPs; % number of random specifications (DGPs)
...
settings.simul.n_MC    = number_MC_reps; % number of Monte Carlo reps
...
% LP-VAR Averaging
settings.est.varlp_est_lambda = 0.5; % weight on LP
settings.est.varlp_est_weights = settings.est.varlp_est_lambda * ones(settings.est.IRF_hor, 1); % set the model average to take the simple average of the two 